...
public class StaticScoreCommands {
                    ...
                    .waitUntil(
                        ()
                            ...
                            : shooter.getRoller(ShooterConstants.FLYWHEEL_ID)
                                  .isRollerAtTargetSpeed(ShotConstants.SUBWOOFER_SPEAKER_SPEED_RPS)
                    )
  ...
  public static Command scoreSpeakerCheckCommand(
    ...
    return Commands.deadline(
        Commands.sequence(
            ...
            Commands
                .waitUntil(
                    ()
                        -> shooter.getRoller(ShooterConstants.FLYWHEEL_ID)
                               .isRollerAtTargetSpeed(ShotConstants.FLYWHEEL_SPEAKER_CHECK_SPEED_RPS
                               )
                )
                .deadlineWith(new LedVariableSpeedCommand(
                    leds,
        ...
        shooter.getRoller(ShooterConstants.FLYWHEEL_ID)
            .startEndRollerRPSWithFFCmd(ShotConstants.FLYWHEEL_SPEAKER_CHECK_SPEED_RPS)
    );
    // .andThen(IntakeCommands.partialStowCommand(intake, shooter));
  }
  ...
  // Drivetrain drivetrain, Intake intake, Shooter shooter, Controlboard
  // controlboard, Leds
  // leds
  // ) {
  ...
  // Commands.deadline(
  // new SafePivotToCommand(
  // intake, shooter, IntakePivotPositions.AMP, ShooterPivotPositions.AMP_PREP,
  // true
  // )
  // .andThen(new PivotToCmd(shooter, ShooterPivotPositions.UNSAFE_AMP, true))
  ...
  // new RollXCmd<Shooter>(shooter,
  // ShooterRollerDistances.AMP_SHOT_DISTANCE),
  // new PivotToCmd(
  ...
  // // )
  // // .andThen(shooter.getRoller().startEndRollerRPSWithFFCmd(
  // // ShotConstants.ROLLER_AMP_RPS
  // // ))
  // new PivotToCmd(shooter, ShooterPivotPositions.UNSAFE_AMP, true)
        ...
        Commands.deadline(
            Commands.sequence(
                Commands.parallel(
                    ...
                    new ElevateToCmd<Climber>(climber, ClimberPositions.AMP2_INIT)
                ),
                // pass note
                Commands.deadline(
                    shooter.getRoller()
                        .startEndRollerRPSCmd(ShotConstants.AMP2_FLYWHEEL_RPS)
                        .until(() -> !shooter.isPieceInChute()),
                    climber.getRoller().startEndRollerRPSCmd(ShotConstants.AMP2_FLYWHEEL_RPS)
                )
            ),
            shooter.getRoller(ShooterConstants.FLYWHEEL_ID)
                .startEndRollerRPSWithFFCmd(ShotConstants.AMP2_FLYWHEEL_RPS)
        ),
        new RollXCmd<Climber>(climber, ClimberRollerDistances.AMP2_PREP),
        Commands.parallel(
            new RollXCmd<Climber>(climber, ClimberRollerDistances.AMP2_PUSH),
            IntakeCommands.stowCommand(intake, shooter),
            new ElevateToCmd<Climber>(
                climber,
                ClimberPositions.AMP2_INTERMEDIATE,
                true,
                ElevatorConstants.ELEVATOR_AMP_MPS
            )
        )
    );
  }
    ...
    return Commands
        .deadline(
            Commands.sequence(
                ...
                Commands.parallel(
                    climber.getRoller()
                        .startEndRollerRPSWithFFCmd(RollerConstants.ROLLER_AMP_SHOT)
                        .withTimeout(0.3),
                    new ElevateToCmd<Climber>(
                        climber,
                        ClimberPositions.AMP2_FINAL,
                        true,
                        ElevatorConstants.ELEVATOR_AMP_MPS
                    )
                )
  ...
  private static Command prepForAmpScoreCommand(
    ...
    return Commands.parallel(
        ...
        // Commands.sequence(
        // shooter.startEndRollerRPSWithFFCmd(ShotConstants.FLYWHEEL_AMP_SPEED_RPS)
        // .until(() -> shooter.isAtTargetSpeed(ShotConstants.FLYWHEEL_AMP_SPEED_RPS)),
        // new RollXCmd<Shooter>(shooter, ShooterRollerDistances.AMP_PREP)
        // ), // TODO: use new rollers command version
        // TODO: replace with safe move
        // new SafePivotToCommand(
        // intake, shooter, IntakePivotPositions.AMP_PREP,
  ...
  private static Command shoot(Shooter shooter, double shotRPS, double timeout) {
    return Commands
        .deadline(
            ...
            shooter.getRoller().startEndRollerRPSWithFFCmd(shotRPS)
        )
        .withTimeout(timeout);
  }
  ...
  public static Command autonomousStaticShots(
    ...
    return Commands
        .deadline(
            Commands.sequence(
                new PivotToCmd(shooter, position, true)
                    .alongWith(AutonomousCommands.postIntakeCommand(intake, shooter))
                    .withTimeout(1.0),
                shoot(shooter, Shooter.ShotConstants.ROLLER_IN_RPS, 1.0)
            ),
            shooter.getRoller(ShooterConstants.FLYWHEEL_ID)
                .startEndRollerRPSWithFFCmd(Shooter.ShotConstants.FLYWHEEL_SPEAKER_SPEED_RPS)
        )
        .finallyDo(
            ()
                -> shooter.getPivot().setPivotProfile(
                    ShooterPivotPositions.PASS_OFF.getRotations(), 0.0
                )
        );
  }
    ...
    return Commands
        .either(
            Commands
                .deadline(
                    Commands.sequence(
                        new PivotToCmd<Shooter>(shooter, position, true)
                            .alongWith(AutonomousCommands.postIntakeCommand(intake, shooter))
                            .withTimeout(1.0),
                        shoot(shooter, Shooter.ShotConstants.ROLLER_IN_RPS, 1.0)
                    ),
                ...
                .andThen(new PivotToCmd(shooter, ShooterPivotPositions.PASS_OFF, true)),
            intake.getRoller()
                .startEndRollerRPSWithFFCmd(IntakeConstants.ROLLER_EJECT_RPS)
                .withTimeout(0.3),
        ...
        .finallyDo(() -> {
          shooter.getPivot().setPivotProfile(ShooterPivotPositions.PASS_OFF.getRotations(), 0.0);
          shooter.getPivot().setFollowProfile(true);
        });
  }
}
