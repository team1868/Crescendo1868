...
public class IntakeCommands {
    ...
    return Commands
        .sequence(
            ...
            Commands.parallel(
                ...
                shooter.getRoller()
                    .startEndRollerRPSWithFFCmd(-20)
                    .until(() -> !shooter.isPieceInChute())
                    .andThen(shooter.getRoller().startEndRollerRPSWithFFCmd(20).until(
                        shooter::isPieceInChute
                    ))
            )
        )
        .deadlineWith(shooter.getRoller(ShooterConstants.FLYWHEEL_ID).startEndRollerRPSWithFFCmd(-8)
        )
  ...
  // // Stage 3 wait until the chute does not see, reverse teh rollers and turn
  // off flywheel,
  // teleop
  // // swerve field relative no other constriant Stage 4 finish when the chute
  // sees it again
  ...
  // true
  // ),
  // shooter.getRoller(ShooterConstants.FLYWHEEL_ID).setFlywheelRPSWithFFCommand(
  ...
  // Commands.deadline(
  // Commands.waitUntil(() -> !shooter.isPieceInChute()),
  // shooter.getRoller(ShooterConstants.FLYWHEEL_ID).startEndRollerRPSWithFFCmd(
  // ShotConstants.FLYWHEEL_SOURCE_SPEED_RPS
  // ),
  // shooter.getRoller().startEndRollerRPSWithFFCmd(
  ...
  public static Command sourceIntakeFeederCommand(
    ...
    // Stage 1 go to the pivot intake positions rollers and flywheel on
    // Stage 2 wait until the chute has seen something
    // Stage 3 wait until the chute does not see, reverse teh rollers and turn off
    // flywheel, teleop
    // swerve field relative no other constriant Stage 4 finish when the chute sees
    // it again
    return Commands
        .sequence(
            Commands.deadline(
                ...
                shooter.getRoller().setRollerRPSWithFFCmd(ShotConstants.ROLLER_SOURCE_SPEED_2_RPS),
                new TeleopSwerveCommand(drivetrain, DriveModes.SNAP_TO_ANGLE)
                    .beforeStarting(
                        ()
            ...
            Commands
                .parallel(
                    ...
                    Commands.sequence(
                        shooter.getRoller()
                            .startEndRollerRPSWithFFCmd(ShotConstants.ROLLER_OUT_RPS)
                            .until(() -> { return !shooter.isPieceInChute(); }),
                        shooter.getRoller()
                            .startEndRollerRPSWithFFCmd(ShotConstants.ROLLER_SLOW_IN_RPS)
                            .until(() -> { return shooter.isPieceInChute(); })
                    )
                )
                .deadlineWith(
        ...
        .finallyDo(() -> {
          ...
          } else if (shooter.getRoller().getPrimaryMotor().getVelocity().getValueAsDouble() <= 5.0) {
    ...
    return Commands.sequence(
        Commands
            .parallel(
                ...
                shooter.getRoller().startEndRollerRPSWithFFCmd(Shooter.ShotConstants.ROLLER_IN_RPS)
            )
            .until(() -> shooter.isPieceInChute()),
        new RollXCmd<Shooter>(
            shooter,
            ShooterRollerDistances.MOVE_NOTE_BACK,
            true,
            shooter.getRoller()._maxVelocityRPS,
  ...
  public static Command clearToScoreCommand(Intake intake) {
    return new PivotToCmd<Intake>(intake, IntakePivotPositions.CLEAR_OF_SHOOTER, true);
  }
}
