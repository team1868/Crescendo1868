...
public class ClimbCommands {
  ...
  private static Command prepToClimbCommand(
    ...
    return Commands.sequence(
        ...
        Commands.parallel(
            new PivotToCmd(climber, ClimberPivotPositions.CLIMB_PREP, true),
  ...
  public static Command engageChainCommand(
    ...
    return Commands
        .sequence(
            new GoToPoseCommand(
                drivetrain,
                drivetrain.getPose().transformBy(new Transform2d()), // TODO:
                                                                     // add
                                                                     // transformation
                xFinalTolerance,
                yFinalTolerance,
  ...
  public static Command elevateCommand(Climber climber, Leds leds) {
    return new PivotToCmd(climber, ClimberPivotPositions.CLIMB, true)
        .deadlineWith(new LedVariableSpeedCommand(
            leds,
            LedModes.LARSON_GREEN,
            ()
                -> (climber.getElevator().getElevatorVelocityMPS()
                    / climber.getElevator()._maxVelocityMPS)
        ));
  }
    ...
    return Commands
        .sequence(
            ...
            new ElevateToCmd<Climber>(
                climber,
                ClimberPositions.TRAP_INTERMEDIATE,
                true,
                ElevatorConstants.ELEVATOR_TRAP_MPS
            ),
            Commands.parallel(
                climber.getRoller()
                    .startEndRollerRPSWithFFCmd(ShotConstants.ROLLER_TRAP_RPS)
                    .withTimeout(ShotConstants.TRAP_TIMEOUT_S),
                new ElevateToCmd<Climber>(
                    climber, ClimberPositions.TRAP_FINAL, true, ElevatorConstants.ELEVATOR_TRAP_MPS
                )
            )
        )
        .deadlineWith(new LedVariableSpeedCommand(
