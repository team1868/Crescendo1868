...
public class AimAllAtSpeakerCommand extends Command {
  ...
  public void execute() {
    ...
    // AlgebraicUtils.placeInScopeRad(_drivetrain.getYaw().getRadians(),
    // goalYawRad)
    // )
    // + "left" + Units.radiansToDegrees(left) + "right" +
    // Units.radiansToDegrees(right) + "cur"
    // + _drivetrain.getYaw().getRadians()
    // );
    // System.out.println(
    ...
    // ffChassisMovement)
    // + " right target deg: "
    // + AlgebraicUtils.placeInScopeRad(
    ...
    if (AlgebraicUtils.placeInScopeRot(
      ...
      if (_choreoAimDriver) {
        _intake.getPivot().setPivotProfile(
            Controlboard.isRedAlliance() ? IntakePivotPositions.RED_GROUND_INTAKE.getRotations()
                                         : IntakePivotPositions.BLUE_GROUND_INTAKE.getRotations(),
            0.0
        );
      } else {
        _intake.getPivot().setPivotProfile(
            IntakePivotPositions.CLEAR_OF_SHOOTER.getRotations(), 0.0
        );
      }
    ...
    // System.out.println("dist to target: " +
  ...
  public void end(boolean interrupted) {
    ...
    if (_activelyShooting || _forceShotMode) {
      _shooter.getRoller().setRollerVelocity(0.0, true);
    }
    if (_activelyShooting || _forceShotMode || _shootingMode || _flywheelOn) {
      _shooter.getRoller(ShooterConstants.FLYWHEEL_ID).setRollerVelocity(0.0, true);
    }
  ...
  public void dealWithChoreoShootingLogistics() {
    ...
    if (!_flywheelOn && (_shootingMode || _forceShotMode)) {
      _shooter.getRoller(ShooterConstants.FLYWHEEL_ID)
          .setRollerVelocity(Shooter.ShotConstants.FLYWHEEL_SPEAKER_SPEED_RPS, true);
      _flywheelOn = true;
    }
    if (!_activelyShooting && (_shootingMode || _forceShotMode)) {
      ...
      if (_forceShotMode
        ...
        _shooter.getRoller().setRollerVelocity(ShotConstants.ROLLER_SPEAKER_RPS, true);
        _activelyShooting = true;
      }
    }
    if (_flywheelOn && !_shootingMode && !_forceShotMode) {
      _flywheelOn = false;
      // _shooter.getRoller(ShooterConstants.FLYWHEEL_ID).setFlywheelControl(0.0,
      // true);
    }
