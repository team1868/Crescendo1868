...
public class ScoreSpeakerOnTheFlyCommand4 extends Command {
  ...
  public void initialize() {
    // _shooter.setRollerPower(ShotConstants.FLYWHEEL_SPEAKER_SPEED_RPS);
    ...
    double TTO = getTravelTimeOutside(
        ...
        Units.rotationsToDegrees(AlgebraicUtils.cppMod(_shooter.getPivot().getPivotPosition(), 1.0))
    );
  ...
  public void end(boolean interrupted) {
    ...
    _shooter.getRoller(ShooterConstants.FLYWHEEL_ID).setRollerPower(0);
  }
  ...
  public void execute() {
    ...
    // );
    // // Translation2d ttoMovement = getNetDistance(_desiredFieldRelativeSpeeds,
    // TTO);

    // Translation2d velocityCompensatedRobot = drivePose.getTranslation();
    // //.plus(ttoMovement);

    // _shooter.setPivotProfileFromTargetPolynomial(velocityCompensatedRobot,
    // _target2d);
    ...
    // && pivotOnTarget(
    // drivePose.getTranslation(), _shooter.getPivotAbsolutePositionRot(),
    // relativeTarget
    // )) {
