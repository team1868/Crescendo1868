...
public class SafePivotToCommand extends Command {
  ...
  public void initialize() {
    _intake.getPivot().setFollowProfile(false);
    _shooter.getPivot().setFollowProfile(false);
    _abort = false;
    ...
    _desiredIntakeRot =
        AlgebraicUtils.placeInScopeRot(_intake.getPivot().getPivotPosition(), _desiredIntakeRot);
    ...
    _desiredShooterRot =
        AlgebraicUtils.placeInScopeRot(_shooter.getPivot().getPivotPosition(), _desiredShooterRot);
    ...
    double shooterPosition = AlgebraicUtils.cppMod(_shooter.getPivot().getPivotPosition(), 1.0);
    _waypoints = getWaypoints(
        getZone(AlgebraicUtils.cppMod(_intake.getPivot().getPivotPosition(), 1.0), shooterPosition),
  ...
  public void execute() {
    ...
    if (_intake.getPivot().isProfileFinished() && _shooter.getPivot().isProfileFinished()) {
      _finished++;
    }
  ...
  public void end(boolean interrupted) {
    if (_abort) {
      _intake.getPivot().setPivotVelocity(0.0, true);
      _shooter.getPivot().setPivotVelocity(0.0, true);
    } else {
      _intake.getPivot().setFollowProfile(_holdPositionAfter);
      _shooter.getPivot().setFollowProfile(_holdPositionAfter);
    }
  }
  ...
  public boolean isFinished() {
    return _abort
        || (_finished > _waypoints.length
            && _intake.getPivot().isPivotInTolerance(_lowerIntakeBound, _upperIntakeBound)
            && _shooter.getPivot().isPivotInTolerance(_lowerShooterBound, _upperShooterBound));
  }
  ...
  private int getIntakeZone(double intakePosition) {
    ...
    var absolute = AlgebraicUtils.cppMod(intakePosition, 1.0);
  ...
  private int getShooterZone(double shooterPosition) {
    ...
    var absolute = AlgebraicUtils.cppMod(shooterPosition, 1.0);
    int i = 0;
