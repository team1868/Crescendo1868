...
public class AimShooterAtTarget extends Command {
  ...
  public void execute() {
    ...
    // - _flywheel.getPrimaryMotor().getVelocity().getValueAsDouble()
    // );
    ...
    // );
    // _ffPivotVelocity =
    // (_goalPivotRot - _shooter.getPivotAbsolutePositionRot()) /
    // Constants.LOOP_PERIOD_S;
    // _shooter.getPrimaryMotor().setControl(_pivotTargetControl.withPosition(_goalPivotRot)
  ...
  public void end(boolean interrupted) {
    ...
    // ShotConstants.FLYWHEEL_SPEAKER_SPEED_RPS
    // -
    // _shooter.getRoller(ShooterConstants.FLYWHEEL_ID).getPrimaryMotor().getVelocity().getValueAsDouble()
    // )
    // );
    _onTarget = false;
  ...
  private static Translation2d getCompensatedGoalLocation2d(Pose2d target, ChassisSpeeds speeds) {
    ...
    ); // TODO are speeds field relative? bc
       // will this work on blue side? eh should
       // be
  }
}
