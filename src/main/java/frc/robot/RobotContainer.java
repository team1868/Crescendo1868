...
public class RobotContainer {
  ...
  public Climber _climber = new Climber(Constants.CRobot._climber);
  public Intake _intake = new Intake(Constants.CRobot._intake);
  public Shooter _shooter = new Shooter(Constants.CRobot._shooter);
  ...
  // private PivotToCommand<Shooter> _shooterStow =
  //     new PivotToCommand(_shooter, ShooterPivotPositions.STOW, true);
  // private PivotToCommand<Shooter> _shooterAmp =
  //     new PivotToCommand(_shooter, ShooterPivotPositions.AMP, true);
  // private PivotToCommand<Shooter> _shooterZero =
  //     new PivotToCommand(_shooter, ShooterPivotPositions.ZERO, true);
  // private PivotToCommand<Shooter> _shooterGround =
  //     new PivotToCommand(_shooter, ShooterPivotPositions.PASS_OFF, true);
  // private PivotToCommand<Shooter> _shooterClear =
  //     new PivotToCommand(_shooter, ShooterPivotPositions.AMP_PREP, true);

  // /* -------- INTAKE PIVOT BRINGUP -------- */
  // private PivotToCommand<Intake> _intakeZero = new PivotToCommand(_intake,
  // IntakePivotPositions.ZERO, true); private PivotToCommand<Intake> _intakeGround =
  //     new PivotToCommand(_intake, Controlboard.isRedAlliance() ?
  //     IntakePivotPositions.RED_GROUND_INTAKE : IntakePivotPositions.BLUE_GROUND_INTAKE, true);
  // private PivotToCommand<Intake> _intakeStow = new PivotToCommand(_intake,
  // IntakePivotPositions.STOW, true); private PivotToCommand<Intake> _intakeClear =
  //     new PivotToCommand(_intake, IntakePivotPositions.CLEAR_OF_SHOOTER, true);

  // /* -------- SAFE PIVOT BRINGUP -------- */
  // // ZONE 0
  // private SafePivotToCommand _zone0 = new SafePivotToCommand(
  //     _intake, _shooter, IntakePivotPositions.CLEAR_OF_SHOOTER, ShooterPivotPositions.AMP_PREP,
  //     true
  // );
  // // ZONE 1
  ...
  //     _intake, _shooter, Controlboard.isRedAlliance() ? IntakePivotPositions.RED_GROUND_INTAKE :
  ...
  //     _intake, _shooter, IntakePivotPositions.STOW, ShooterPivotPositions.STOW, true
  // );
  ...
  // private PivotToCommand<Intake> _intakeGroundPivot =
  //     new PivotToCommand(_intake, Controlboard.isRedAlliance() ?
  //     IntakePivotPositions.RED_GROUND_INTAKE : IntakePivotPositions.BLUE_GROUND_INTAKE, true);
  // private PivotToCommand<Intake> _intakeStowPivot =
  //     new PivotToCommand(_intake, IntakePivotPositions.STOW, true);
  // private PivotToCommand<Intake> _intakePartialStowPivot =
  //     new PivotToCommand(_intake, IntakePivotPositions.PARTIAL_STOW, true);

  private Command _shooterRollerFF = Commands.startEnd(() -> {
    _shooter.getRoller().setRollerVelocityControl(90, true);
    System.out.println(
        "\t curPivotAngleDeg "
        + (360.0 - Units.rotationsToDegrees(_shooter.getPivot().getPivotAbsolutePositionRot()))
        + "\t cur flywheel vel" + _shooter.getFlywheel().getFlywheelMotor().getVelocity()
    );
  }, () -> _shooter.getRoller().setRollerPower(0.0));

  private Command _intakeRollerFF = _intake.getRoller().startEndRollerRPSWithFFCommand(90);
  private Command _flywheelRollerFF = _shooter.getRoller().startEndRollerRPSWithFFCommand(-70);

  private Command _flywheelWhileOnCommand = _shooter.getFlywheel().setFlywheelPowerCommand(0.5);
  private Command _setFlywheelRPSCommand = _shooter.getFlywheel().setFlywheelRPSCommand(50);
  private Command _setFlywheelRPSwFFCommand =
      _shooter.getFlywheel().setFlywheelRPSWithFFCommand(50);

  // private RollerRotateXCommand<Shooter> _shooterRotateXCommand =
  //     new RollerRotateXCommand<Shooter> (_shooter, 0.25, 0.1, 0.1);
  private RollerRotateXCommand<Shooter> _shooterRotateXCommand =
      new RollerRotateXCommand<Shooter>(_shooter, 20.0, 0.1, 0.1);
  private Command _feedUntilLimit = _shooter.getRoller().startEndRollerRPSWithFFCommand(50.0).until(
      () -> _shooter.isPieceInChute()
  );

  // Climber
  private ElevatorRaiseToCommand _climberRaiseToCommand =
      new ElevatorRaiseToCommand(_climber, ClimberPositions.AMP2_INIT, true);
  private ElevatorRaiseToCommand _climbRaiseToTrap =
      new ElevatorRaiseToCommand(_climber, ClimberPositions.TRAP_POSITION, true);
  ...
  //     StaticScoreCommands.scoreAmpCommand(_drivetrain, _intake, _shooter, _controlboard, _leds);
  ...
  private ElevatorRaiseToCommand<Climber> _climberStowCommand =
      new ElevatorRaiseToCommand<Climber>(_climber, ClimberPositions.STOW, true);
  private ElevatorRaiseToCommand<Climber> _climberExtendCommand =
      new ElevatorRaiseToCommand<Climber>(_climber, ClimberPositions.EXTEND, true);
  private Command _raiseHooksClimbCommand = new ElevatorRaiseToCommand<Climber>(
      Climber.ElevatorConstants.HOOKS_ID, _climber, ClimberPositions.CLIMB_EXTEND
  );
  private Command _lowerHooksClimbCommand = new ElevatorRaiseToCommand<Climber>(
  ...
  //     new ElevatorRaiseToCommand(Climber.ElevatorConstants.HOOKS_ID, _climber,
  //     HookPositions.ZERO)
  //         .beforeStarting(
  //             ()
  ...
  protected void configShuffleboard() {
    ...
    // Drivetrain
    // systemCheckTab.add("Set Pose ", _setPoseCommand).withPosition(6, 3).withSize(2, 1);
    ...
    // systemCheckTab.add("Score Fly ", _onTheFlySpeaker).withPosition(9, 1).withSize(2, 1);
  }
  ...
  public void periodic() {
    // controlboard is not a subsystem so the periodic function must be explicitly called
  ...
  private void configureTestBindings() {
    ...
    // Controlboard._xboxDrive.rightTrigger().toggleOnTrue(_scoreSpeakerCommand);
    // Controlboard._xboxDrive.leftTrigger().toggleOnTrue(_ejectCommand); // no vision ground
    // intake yet
