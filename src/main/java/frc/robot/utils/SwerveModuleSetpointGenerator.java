...
public class SwerveModuleSetpointGenerator {
  ...
  /**
   * Returns module states that result in ChassisSpeeds as close to
   * |desiredChassisSpeeds| as
   * possible without exceeding |allowedScrub|.
   *
   * @param desiredChassisSpeeds Chassis speeds to try to achieve while staying in
   *                             scrub and
   *                             feasibility limits.
  ...
  public SwerveSetpoint getFeasibleSetpoint(
    ...
    // Binary search between linearly interpolated current and desired chassis
    // speeds that doesn't
  ...
  /**
   * Returns the SwerveSetpoint for the given |chassisSpeeds| after desaturating
   * wheel speeds.
   */
  ...
  /**
   * Returns the amount of scrub for each module relative to overall chassis
   * movement.
   */
  public double computeMaxModuleScrubs(final SwerveModuleState[] states) {
    ...
    for (int i = 0; i < states.length; i++) {
      var cart1 = AlgebraicUtils.pol2cart(
          idealStates[i].speedMetersPerSecond, idealStates[i].angle.getRadians()
      );
      var cart2 =
          AlgebraicUtils.pol2cart(states[i].speedMetersPerSecond, states[i].angle.getRadians());
   ...
   * by potentially
   * reversing the direction the wheel spins. Places the result within [-pi, pi)
   * of the current
  ...
  private static SwerveModuleState optimizeModule(
    ...
    double targetAngle = AlgebraicUtils.placeInScopeRad(
        desiredState.angle.getRadians(), currentState.angle.getRadians()
    );
